...
const gchar *gqr_logging_string(GLogLevelFlags level)
{
  ...
  if (G_LOG_LEVEL_ERROR & level)
    return strings[2];
  if (G_LOG_LEVEL_CRITICAL & level)
    return strings[3];
  if (G_LOG_LEVEL_WARNING & level)
    return strings[4];
  if (G_LOG_LEVEL_MESSAGE & level)
    return strings[5];
  if (G_LOG_LEVEL_INFO & level)
    return strings[6];
  if (G_LOG_LEVEL_DEBUG & level)
    return strings[7];
                      ...
                      const gchar * message, gpointer data[])
{
  ...
  fprintf(f, "%s%s-%s: %s\n", p,
          G_LOG_DOMAIN, gqr_logging_string(log_level), message);
...
gint gqr_logging_init(FILE * f, gchar * p,
                      GLogLevelFlags log_level, gpointer exit_func)
{
  static gpointer data[GQR_LOGGING_DATA_WIDTH];
  static GLogLevelFlags level;

  if (f != NULL)
    data[GQR_LOGGING_DATA_FID] = f;
  else
    data[GQR_LOGGING_DATA_FID] = stderr;

  if (p != NULL)
    data[GQR_LOGGING_DATA_PREFIX] = g_strdup(p);
  else
    data[GQR_LOGGING_DATA_PREFIX] = g_strdup("");
  ...
  g_log_set_handler(G_LOG_DOMAIN,
                    ...
                    G_LOG_LEVEL_MESSAGE |
                    G_LOG_LEVEL_INFO |
                    G_LOG_LEVEL_DEBUG, (GLogFunc) gqr_logging_func, data);
