                            ...
                            gdouble * x, gdouble * w)
{
  ...
  for (i = 1; i <= m; i++) {
    Py[i + 1] =
      ((gdouble) (2 * i + 1) * y * Py[i] -
       (gdouble) i * Py[i - 1]) / (gdouble) (i + 1);
  }
  ...
  if ((y == 1.0) || (y == -1.0))
    for (i = 1; i < m; i++)
      dPy[i + 1] =
        y * gsl_pow_int(y, i + 1) * 0.5 * (gdouble) ((i + 1) * (i + 2));
  else
    for (i = 1; i < m; i++)
  ...
  for (i = 0; i < m; i++) {
    ...
    for (j = 1; j < m; j++) {
      ...
      M[3 * m + j] +=
        gqr_rule_weight(h,
                        i) * (Pn - Py[j] + dPy[j] * (y - t)) / (y - t) / (y -
                                                                          t);
      Pnp1 =
        ((gdouble) (2 * j + 1) * gqr_rule_abscissa(h, i) * Pn -
