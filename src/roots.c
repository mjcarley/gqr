				    ...
				    gdouble d2p, gdouble d2q, gdouble d2r)
{
  ...
  U[0] = u ; U[1] = du ; U[2] = -(q*U[1] + r*U[0])/p ;
  U[3] = -(dp*U[2] + dq*U[1] + q*U[2] + dr*U[0] + r*U[1])/p ;

  for ( k = 2 ; k < n ; 
	(U[k+2] = -((k*dp+q)*U[k+1] + (0.5*k*(k-1)*d2p + k*dq + r)*U[k] +
		    (0.5*k*(k-1)*d2q + k*dr)*U[k-1] +
		    0.5*k*(k-1)*d2r*U[k-2])/p), (k ++) ) ;
...

{
  ...
  for ( (x[0] = x[1] = x0), (du[0] = du0), (i = 1) ; i < N ; 
    ...
    gqr_taylor_coefficients(U, GQR_TAYLOR_LENGTH, 0.0, du[i-1],
			    GQR_POLYVAL(P,x[i-1]), GQR_POLYVAL(Q,x[i-1]),
			    GQR_POLYVAL(R,x[i-1]),
			    GQR_POLYDER(P,x[i-1]), GQR_POLYDER(Q,x[i-1]),
			    GQR_POLYDER(R,x[i-1]),
			    GQR_POLYDER2(P,x[i-1]), GQR_POLYDER2(Q,x[i-1]),
			    GQR_POLYDER2(R,x[i-1])) ;
    gqr_newton_solve(U, GQR_TAYLOR_LENGTH, x[i-1], 
		      tol, &x[i], &du[i]) ;
  }
